...
public abstract class MySwipeHelper extends ItemTouchHelper.SimpleCallback {
    ...
    private Queue<Integer> removewQueue;
        ...
        public boolean onTouch(View v, MotionEvent event) {
            ...
            if (event.getAction() == MotionEvent.ACTION_DOWN ||
                ...
                if (rect.top < point.y && rect.bottom > point.y)
                    ...
                    removewQueue.add(swipePosition);
                    swipePosition = -1;
                    recoverSwipedItem();
                }
            }
    ...
    public MySwipeHelper(Context context, RecyclerView recyclerView, int buttonWidth) {
        ...
        removewQueue = new LinkedList<Integer>() {
            @Override
    ...
    private synchronized void recoverSwipedItem() {
        while (!removewQueue.isEmpty()) {
            int pos = removewQueue.poll();
            if (pos > -1)
    ...
    public class MyButton {
        ...
        public boolean onClick(float x, float y) {
            if (clicckRegion != null && clicckRegion.contains(x, y)) {
